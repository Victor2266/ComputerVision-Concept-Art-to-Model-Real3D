...
def main():
    ...
    for epoch in range(start_ep, end_ep):
        ...
        #         'eval_dict': metrics,
        #     }, save_path=save_path)
        # Save best model based on main validation metrics
        
        cur_psnr = metrics.get('psnr', 0.0)  # Using PSNR as main metric
        cur_sim = metrics.get('nvs_clip_all', 0.0)
        ...
        if cur_psnr > best_psnr:
            ...
            if config.train.use_zeroRO:
                print('Consolidated on rank {} because of ZeRO'.format(local_rank))
                optimizer.consolidate_state_dict(0)
            save_path = os.path.join(output_dir, "cpt_best_psnr_{}_sim_{}.pth.tar".format(best_psnr, cur_sim))
            dist_utils.save_on_master({
                'epoch': epoch + 1,
                'state_dict': model.state_dict(),
                'optimizer' : optimizer.state_dict() if dist_utils.is_main_process() else None,
